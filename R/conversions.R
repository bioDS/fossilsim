...
sampled.tree.from.combined <- function(tree, rho = 1, sampled_tips = NULL) {
  if (!("SAtree" %in% class(tree))) {
    if ("phylo" %in% class(tree)) {
      ...
      stop(paste('object "', class(tree), '" is not of class "SAtree"', sep = ""))
    }
  }
...
prune.fossils <- function(tree) {
  ...
  for (name in unique(split_names[, 1])) {
    ...
    mx <- max(as.integer(split_names[idx, 2]))
    mn <- min(as.integer(split_names[idx, 2]))
    for (id in idx) {
      if (as.integer(split_names[id, 2]) == mn) {
        tree$tip.label[id] <- paste0(name, "_first")
      } # 1 corresponds to oldest sample in that lineage
      else if (mx > 1 && as.integer(split_names[id, 2]) == mx) {
        tree$tip.label[id] <- paste0(name, "_last")
      } # earliest sample
      else {
...
#'     p = 0.1, q = 0.1, r = 0.1, nruns = 1, nTotalTaxa = c(30, 40),
...
paleotree.record.to.fossils <- function(record) {
  ...
  fossildf <- fossils()
  taxonomy <- data.frame(
    edge = numeric(), sp = numeric(), start = numeric(), end = numeric(), mode = character(),
  ...
  for (i in 1:length(record)) {
    ...
    if (is.na(record[[i]]$taxa.data[["ancestor.id"]])) { # root species
      ...
      if (length(orig_edge) == 0 || tree$edge.length[orig_edge] > 0) {
        ...
        if (any(tree$edge.length[desc_anc] == 0)) {
          mode <- "a"
        } # anagenetic event
        else {
          mode <- "b"
    ...
    taxonomy <- rbind(taxonomy, data.frame(
      sp = names(record)[i], edge = all_nodes, parent = parent,
    ...
    if (length(record[[i]]$sampling.times) > 0) {
      fossildf <- rbind(fossildf, data.frame(
        sp = names(record)[i], edge = sampled_nodes,
...
fossils.to.paleotree.record <- function(fossils, tree = NULL, taxonomy = NULL) {
  ...
  if (!is.null(taxonomy)) {
    ...
    for (i in 1:length(species$sp)) {
      ...
      record[[i]] <- list(
        taxa.data = c(i, anc, species$species.start[i], species$species.end[i], (species$species.end[i] < 1e-5), which(names(record) == species$cryptic.id[i])),
    ...
    .convert_one_species <- function(current_node, ancestor, record) {
      if (current_node <= length(tree$tip.label)) {
        current_species <- as.character(tree$tip.label[current_node])
      } else if (!is.null(tree$node.label)) {
        current_species <- as.character(tree$node.label[current_node - length(tree$tip.label)])
      } else {
        current_species <- paste0("t", current_node)
      }

      above_node <- tree$edge[which(tree$edge[, 2] == current_node), 1]
      record[[current_species]] <- list(
        taxa.data = c(length(record) + 1, ancestor, node.ages[above_node], node.ages[current_node], (node.ages[current_node] < 1e-3), length(record) + 1),
        sampling.times = numeric()
      )
...
#' f <- sim.fossils.intervals(
#'   tree = t, interval.ages = times, rates = rates,
...
fossils.to.pyrate <- function(fossils, python = TRUE, traits = NULL, cutoff = NULL, random = FALSE, min = NULL,
  ...
  if (!is.null(cutoff) && cutoff > 0) {
    ...
    if (length(fossils$sp) < 1) {
      stop("Number of specimens after cutoff must be > 0")
  ...
  if (python) {
    ...
    if (random && any(fossils$hmax != fossils$hmin)) {
      fossils$hmin <- round(unlist(lapply(1:dim(fossils)[1], function(x) {
        runif(1, fossils$hmin[x], fossils$hmax[x])
      })), digits = 6)
      fossils$hmax <- fossils$hmin
    } else if (any(fossils$hmax != fossils$hmin)) { # use median ages
      fossils$hmin <- round(unlist(lapply(1:dim(fossils)[1], function(x) {
        mean(c(fossils$hmin[x], fossils$hmax[x]))
      })), digits = 6)
      fossils$hmax <- fossils$hmin
    } else {
